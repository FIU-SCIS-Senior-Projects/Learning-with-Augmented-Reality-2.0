Shader "Custom/ColorDebug"
{
	...
	{
		Tags { "Queue"="Geometry" }
		ZWrite On	//should we write to the depth buffer?

		//Blend modes equation:
		//	SrcFactor * fragment_output + DstFactor * pixel_color_in_framebuffer;
		//Blen{code for SrcFactor} {code for DstFactor}

		Blend SrcAlpha OneMinusSrcAlpha	//alpha blending
		//Blend One OneMinusSrcAlpha	//premultiplied alpha blending
		//Blen One One					//additive
		//Blend SrcAlpha One			//additive blending
		//Blend OneMinusDstColor One	//soft additive
		//Blend DstColor Zero			//multiplicative
		//Blend DstColor SrcColor		//2x multiplicative
		//Blend Zero SrcAlpha			//multiplicative blending for attenuation by the fragments's alpha
		...
		Pass
		{
			...
			struct vertexInput
			{
				...
				float4 texcoord1 : TEXCOORD1;	//1st set of texture coordinates (aka "UV"; between 0 and 1) 
			...
			fragmentInput vert(vertexInput i)
			{
				...
				//debug: uncomment the desired item to debug them and then return i.color directly in the frgment shader
				//o.color = i.texcoord;
				//o.color = i.texcoord1;
				//o.color = i.vertex;
