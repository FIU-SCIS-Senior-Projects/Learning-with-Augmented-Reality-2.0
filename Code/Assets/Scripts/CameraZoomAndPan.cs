...
public class CameraZoomAndPan : MonoBehaviour
{
    ...
    // ýêðàííûå êîîðäèíàòû íà÷àëüíîé òî÷êè êàñàíèÿ
    private Vector3 initialTouchPosition;
    // ìèðîâûå êîîðäèíàòû êàìåðû ïðè èíèöèèðîâàíèè
    // ïåðåìåùåíèÿ/ìàñøòàáèðîâàíèÿ
    private Vector3 initialCameraPosition;

    // ýêðàííûå êîîðäèíàòû íà÷àëüíîé òî÷êè ïåðâîãî êàñàíèÿ
    private Vector3 initialTouch0Position;
    // ýêðàííûå êîîðäèíàòû íà÷àëüíîé òî÷êè âòîðîãî êàñàíèÿ
    private Vector3 initialTouch1Position;
    // ñðåäíÿÿ òî÷êà ìåæäó íà÷àëüíûìè êîîðäèíàòàìè êàñàíèé
    private Vector3 initialMidPointScreen;
    // îðòîãîíàëüíûé ðàçìåð êàìåðû íà ìîìåíò íà÷àëà ìàñøòàáèðîâàíèÿ
    ...
    void Update()
    {
        if (Input.touchCount == 1)
        {
            ...
            if (IsTouching(touch0))
            {
                ...
                {
                    Vector2 delta = GetComponent<Camera>().ScreenToWorldPoint(touch0.position) -
                                    GetComponent<Camera>().ScreenToWorldPoint(initialTouchPosition);
        ...
        if (Input.touchCount == 2)
        {
            ...
            else
            {
                ...
                Vector3 initialMidPointWorldBeforeZoom = GetComponent<Camera>().ScreenToWorldPoint(initialMidPointScreen);

                Camera.main.orthographicSize = initialOrthographicSize / scaleFactor;

                Vector3 initialMidPointWorldAfterZoom = GetComponent<Camera>().ScreenToWorldPoint(initialMidPointScreen);
                Vector2 initialMidPointDelta = initialMidPointWorldBeforeZoom - initialMidPointWorldAfterZoom;

                Vector2 oldAndNewMidPointDelta =
                    GetComponent<Camera>().ScreenToWorldPoint(currentMidPoint) -
                    GetComponent<Camera>().ScreenToWorldPoint(initialMidPointScreen);
