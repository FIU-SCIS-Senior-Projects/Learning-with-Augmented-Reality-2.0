...
public class Loader : MonoBehaviour
{
    ...
    void LoadXML()
    {
        ...
        xmlDoc = XDocument.Load("Assets/Resources/ XML - Test(export).xml");

        //This basically breaks down the XML Document into XML Elements. Used later. 
        items = xmlDoc.Descendants(tag).Elements ();
    }
...
/*
    if(item.Parent.Attribute(“page number”).Value == iteration.ToString())
    {
        pageNum = int.Parse(item.Parent.Attribute (“page number”).Value);
